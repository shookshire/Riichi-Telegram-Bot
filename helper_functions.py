...
def is_renchan(hand):
  ...
  if (hand['outcome'] == 'Draw' or hand['outcome'] == 'Nagashi Mangan') and hand['tenpai'][hand['round num'] - 1]:
    return True
...
def process_game(game):
  game['final score'] = game['hands'][-1]['final score'] if len(
      game['hands']) else [game['initial value']]*4
  game['position'] = game['hands'][-1]['position'] if len(game['hands']) else [
      2.5, 2.5, 2.5, 2.5]
...
def get_next_hand_wind_round_honba(prev_hand):
  ...
  if is_renchan(prev_hand):
    ...
    if prev_hand['outcome'] == 'Draw' or prev_hand['outcome'] == 'Nagashi Mangan':
      honba += 1
    else:
      honba = 0
...
def get_total_score_change(hand, kiriage):
  # score_change = get_tsumo_score_change(hand, hand['honba'], kiriage) if hand['outcome'] == 'Tsumo' else get_ron_score_change(
  #     hand, hand['honba'], kiriage) if hand['outcome'] == 'Ron' else get_draw_score_change(hand) if hand['outcome'] == 'Draw' else [0, 0, 0, 0]
  score_change = [0, 0, 0, 0]
  if hand['outcome'] == 'Tsumo' or hand['outcome'] == 'Nagashi Mangan':
    score_change = get_tsumo_score_change(hand, hand['honba'], kiriage)
  if hand['outcome'] == 'Ron':
    score_change = get_ron_score_change(hand, hand['honba'], kiriage)
  if hand['outcome'] == 'Draw':
    score_change = get_draw_score_change(hand)
...
def get_tsumo_value(han, fu, oya, kiriage):
  return get_value_list('Tsumo', oya, kiriage)[han][fu] if re.match(
      '^[1-4]$', han) else get_value_list('Tsumo', oya, kiriage)[han]
...
def get_tsumo_score_change(hand, honba, kiriage):
  ...
  if is_oya_win(hand):
    value = 40 if hand['outcome'] == 'Nagashi Mangan' else get_tsumo_value(
        hand['han'], hand['fu'], True, kiriage
    )
    for i in range(4):
      if not i == winner:
        score_change[i] = -value
        if hand['outcome'] == 'Tsumo':
          score_change[i] -= honba
    score_change[winner] = -sum(score_change)
  else:
    oya_value = 40 if hand['outcome'] == 'Nagashi Mangan' else get_tsumo_value(
        hand['han'], hand['fu'], True, kiriage
    )
    value = 20 if hand['outcome'] == 'Nagashi Mangan' else get_tsumo_value(
        hand['han'], hand['fu'], False, kiriage
    )
    for i in range(4):
      if not i == winner:
        score_change[i] = -value
        if hand['outcome'] == 'Tsumo':
          score_change[i] -= honba
    score_change[hand['round num'] - 1] = -oya_value
    if hand['outcome'] == 'Tsumo':
      score_change[hand['round num'] - 1] -= honba
    score_change[winner] = -sum(score_change)
...
def get_ron_score_change(hand, honba, kiriage):
  ...
  if is_oya_win(hand):
    value = get_value_list('Ron', True, kiriage)[hand['han']][hand['fu']] if re.match(
        '^[1-4]$', hand['han']) else get_value_list('Ron', True, kiriage)[hand['han']]
  else:
    value = get_value_list('Ron', False, kiriage)[hand['han']][hand['fu']] if re.match(
        '^[1-4]$', hand['han']) else get_value_list('Ron', False, kiriage)[hand['han']]
...
def get_individual_outcome(hand):
  ...
  if hand['outcome'] == 'Nagashi Mangan':
    ...
    outcome[hand['winner idx']] = 'Nagashi-win'
    return outcome
...
def print_hand_title(hand):
  ...
  text += '`Round: {}{}  {} Honba\n\n`'.format(
      hand['wind'], hand['round num'], hand['honba'])
  return text
...
def print_hand_settings(hand, player_names):
  ...
  if hand['outcome'] == 'Draw':
    ...
    text += '`\n`'
  elif hand['outcome'] == 'Nagashi Mangan':
    text += '`{} Won Nagashi Mangan\n\n`'.format(
        player_names[hand['winner idx']]
    )
  else:
    if hand['outcome'] == 'Tsumo':
      ...
      text += '`{} Ron {}\n`'.format(
          player_names[hand['winner idx']], player_names[hand['loser idx']]
      )
...
def print_score_change(hands, player_names):
  ...
  for i in range(1, len(hands) + 1):
    if hands[-i]['hand num'] == hand_num:
      score_change = [x + y for x,
                      y in zip(score_change, hands[-i]['score change'])]
...
def print_current_game_settings(game):
  ...
  text += '`Aka:            | {}\n`'.format(game['aka'])
  text += '`Uma:            | {},{},{},{}\n`'.format(
      game['uma'][0], game['uma'][1], game['uma'][2], game['uma'][3])
  text += '`Oka:            | {}\n`'.format(game['oka'])
  text += '`Chombo Value:   | {}\n`'.format(game['chombo value'])
  text += '`Chombo Option:  | {}\n`'.format(game['chombo option'])
  text += '`Kiriage Mangan: | {}\n`'.format(
      'Yes' if game['kiriage'] else 'No')
  text += '`Multiple Ron: 	 | {}\n`'.format(
      'Yes' if not game['atamahane'] else 'No')
